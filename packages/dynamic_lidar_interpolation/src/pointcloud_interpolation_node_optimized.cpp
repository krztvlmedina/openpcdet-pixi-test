/**
 * @file pointcloud_interpolation_node_optimized.cpp
 * @brief OPTIMIZED ROS2 Node for LiDAR Point Cloud Interpolation with Minimal Latency.
 *
 * This is an optimized version of the interpolation node with:
 * - Reduced queue depths (10 -> 1) for minimal latency
 * - Frame dropping when busy (similar to PCDet node)
 * - Fixed QoS reliability consistency bug (line 601)
 * - Non-blocking callback design
 *
 * Key optimizations:
 * 1. Queue depth 1 instead of 10 (lines 197, 209) - eliminates message backlog
 * 2. Frame skipping when processing (prevents stale data processing)
 * 3. Consistent BEST_EFFORT QoS (fixed line 601 bug)
 * 4. KeepLast(1) on dynamic topic changes (line 619)
 *
...
/**
 * @brief OPTIMIZED ROS2 Node for LiDAR Point Cloud Interpolation.
 *
 * Optimizations:
 * - Frame dropping when busy to prevent stale data processing
 * - Minimal queue depths (1) for lowest latency
...
public:
    LidarInterpolationNode()
        : Node("lidar_interpolation_node"),
          processing_frame_(false)  // OPTIMIZATION: Track processing state
    {
        RCLCPP_INFO(this->get_logger(), "Initializing OPTIMIZED LidarInterpolationNode...");
        ...
        RCLCPP_INFO(this->get_logger(), "OPTIMIZED Lidar Interpolation Node Initialized.");
        RCLCPP_INFO(this->get_logger(), "Frame dropping enabled for minimal latency.");
    }
...
private:
     ...
     * OPTIMIZATION: Changed KeepLast(10) to KeepLast(1) for minimal latency
     */
    void initialize_publishers()
    {
        // PointCloud2 Publisher with Best Effort QoS
        // OPTIMIZED: KeepLast(1) instead of KeepLast(10)
        interpolated_point_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
            interpolated_point_cloud_topic_,
            rclcpp::QoS(rclcpp::KeepLast(1)).best_effort());

        RCLCPP_INFO(this->get_logger(), "Initialized PointCloud2 publisher for '%s' with KeepLast(1).",
    ...
    void initialize_subscribers()
    {
        ...
        // OPTIMIZED: KeepLast(1) instead of KeepLast(10)
        rclcpp::QoS qos_lidar = rclcpp::QoS(rclcpp::KeepLast(1)).best_effort();
        sub_lidar_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            lidar_topic_,
            qos_lidar,
            std::bind(&LidarInterpolationNode::fusionCallback, this, std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(), "Subscribed to LiDAR topic '%s' with KeepLast(1).",
                    lidar_topic_.c_str());
    }
     ...
     *
     * OPTIMIZATION: Added frame dropping logic to skip stale messages when busy.
     * Similar to PCDet node behavior.
     *
    ...
    {
        // OPTIMIZATION: Non-blocking frame skip check
        ...
        try
        {
            ...
            if (!interpolated_dense_cloud || interpolated_dense_cloud->points.empty())
            {
                ...
                processing_frame_ = false;  // OPTIMIZATION: Reset flag before return
                return;
            }
            ...
            // OPTIMIZATION: Log statistics every 100 frames
        ...
        processing_frame_ = false;  // OPTIMIZATION: Reset processing flag
    }
        ...
        for (const auto &param : parameters)
        {
            try
            {
                ...
                else if (param.get_name() == "topics.lidar_topic")
                {
                    ...
                    // OPTIMIZATION: KeepLast(1) and best_effort() for consistency
                    sub_lidar_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
                        lidar_topic_,
                        rclcpp::QoS(rclcpp::KeepLast(1)).best_effort(),  // FIXED: was reliable(), now best_effort()
                        std::bind(&LidarInterpolationNode::fusionCallback, this, std::placeholders::_1));
                ...
                else if (param.get_name() == "topics.interpolated_point_cloud_topic")
                {
                    ...
                    // OPTIMIZATION: KeepLast(1) instead of KeepLast(10)
                    interpolated_point_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
                        interpolated_point_cloud_topic_,
                        rclcpp::QoS(rclcpp::KeepLast(1)).best_effort());  // FIXED: KeepLast(1) instead of KeepLast(10)
    ...
    // OPTIMIZATION: Frame dropping tracking
    std::atomic<bool> processing_frame_;
    std::atomic<size_t> frames_processed_{0};
    std::atomic<size_t> frames_skipped_{0};
};
